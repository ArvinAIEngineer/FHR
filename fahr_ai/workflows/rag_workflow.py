...
class RAGWorkflow(BaseWorkflow):
    ...
    def __init__(
        ...
        with open(config_path, "r") as file:
            ...
            # topic_file_path: dict of Paths to the YAML file containing predefined topics
            # Load Arabic topics from YAML file
            with open(self.config["topic_file_path"]["ar"], 'r', encoding='utf-8') as file:
                topics_data = file.readlines()
            self.AR_topics_list = [topic.strip() for topic in topics_data]
            # Load English topics from YAML file
            with open(self.config["topic_file_path"]["en"], 'r', encoding='utf-8') as file:
                topics_data = file.readlines()
            self.EN_topics_list = [topic.strip() for topic in topics_data]
    ...
    def retrieve_content_from_vector_standard(self, query, fallback_attempts=3):
    ...
    def retrieve_by_topic(self, query, min_docs=3):
        """
        Standalone topic-based retrieval that retrieves content based on the identified topic.
        Uses the existing 'topic' metadata field for filtering documents.
        ...
        Returns:
            Documents retrieved based on topic
        """
        # First, classify the query to identify the relevant topic
        relevant_topics = self.classify_query_topic(query)
        if not relevant_topics:
            return None
        else:
            self.base_logger.info(f"Retrieved topics for query: {relevant_topics}, {type(relevant_topics)}")

            # Use a metadata filter to specifically find documents with the matching topic
            filter_dict = {"topic": relevant_topics}

            # Retrieve documents with the matching topic metadata
            topic_docs = []
            for topic in relevant_topics:
                res = self.vectorstore._collection.get(
                    include=["documents", "metadatas"],
                    where={"topic": topic.strip()}
                )
                
                # Iterate over each document and metadata
                for doc_text, meta in zip(res["documents"], res["metadatas"]):
                    topic_docs.append(Document(page_content=doc_text, metadata=meta))
            # Format the retrieved documents
            formatted_docs = self.format_docs(topic_docs)

            self.base_logger.info(f"Retrieved {len(formatted_docs)} documents based on topic: {relevant_topics}")

            return {
                "docs": formatted_docs,
                "topic": relevant_topics
            }

    def retrieve_content_from_vector(self, query):
        """
        Enhanced retrieval that combines results from standard query-based retrieval
        ...
        """
        # Perform standard retrieval using the original method
        standard_results = self.retrieve_content_from_vector_standard(query)
        all_docs = standard_results[:2]

        # Perform topic-based retrieval
        topic_results = self.retrieve_by_topic(query)
        if topic_results is not None:
            # Merge the results, removing duplicates
            topic_docs = topic_results.get("docs", [])
            # Create a set of existing document IDs to avoid duplicates
            existing_chunkTexts = {doc["chunkText"]
                            for i, doc in enumerate(all_docs)}

            # Add unique documents from topic-based retrieval
            for doc in topic_docs:
                chunkText = doc["chunkText"]
                if chunkText is not None and chunkText not in existing_chunkTexts:
                    all_docs.append(doc)

        self.base_logger.info(f"Combined retrieval: {len(all_docs)} unique documents")

        return all_docs


    def classify_query_topic(self, query: str):
        """
        Classifies the query into the most relevant topic(s) from the predefined topics list.
        """
        ...
        try:
            ...
            if language == "ar":
                topics_list = self.AR_topics_list
            elif language == "en":
                topics_list = self.EN_topics_list

            # Format topics in a numbered list for clarity
            topics_formatted = ", ".join(topics_list)

            # Build prompt directly as a string
            classification_prompt = f"""
            You are an classifier on FAHR (Federal Authority for Government Human Resources) regulations and UAE labor laws. The user query is:

            "{query}"

            Choose the MOST 3 RELEVANT topics from the following list:
            {topics_formatted}

            Return only the topic names, comma separated.
            """

            # Call the LLM (string input, no HumanMessage)
            self.base_logger.info("Invoking with LLM direct...")
            start_time = time.time()
            response = self.model.invoke(classification_prompt)
            end_time = time.time()
            self.base_logger.info(f"LLM response received in {end_time - start_time:.2f} seconds")
            
            # Split into list
            self.base_logger.info("Processing response...")
            identified_topics = re.sub(r"<think>.*?</think>", "", response.content, flags=re.DOTALL).strip()
            identified_topics = identified_topics.split(",")

            return identified_topics if identified_topics else []

        except Exception as e:
            self.base_logger.error(f"Error classifying query: {str(e)}")
            return []
    ...
    def build_workflow(self) -> Any:
            ...
            if original_query:
                ...
                if retrieve_context:
                    ...
                    if isinstance(result, dict) and result.get("context", "").strip() == "[NO_DOCUMENTS_FOUND]":
                        ...
                        original_query = memory.get("original_query", original_query)
                        result = retrieve_context.invoke(original_query)
        ...
        self.workflow.add_edge(START, "initial_retrieval")  # Start with query refinement
    ...
    def run(self, state: Dict[str, Any], config: dict) -> Dict[str, Any]:
        """
        ...
        print(f"RAGWorkflow RUN state keys: {list(state.keys())}")
        print(f"RAGWorkflow RUN message count: {len(state.get('messages', []))}")

        #  Fix is here: copy messages and memory from input
        messages_state = {
            "messages": state.get("messages", []),
            "memory": state.get("memory", {}),
        ...
        if "reference_data" in result:
            state["citations"] = result["reference_data"]
...
if __name__ == "__main__":
    ...
    llm=ChatOpenAI(
                ...
                openai_api_base=configs["openai_api_base_url"],
    ...
    rag_workflow_instance = RAGWorkflow(vectorstore=vectorstore, llm_model=llm)
    message_objs = [HumanMessage(content="what are the types of leave")]
    state = {
        "messages": message_objs,
        "conversation_id": "223"
    }

    config = RunnableConfig(configurable={"thread_id": 223})
    result = rag_workflow_instance.run(state, config=config)
    print("#####################################")
    print(result)
