...
class ReviewerAgent(BaseAgent):
    ...
    async def run(self, state: GraphState, config: RunnableConfig) -> GraphState:
        ...
        def get_recent_ai_messages(messages, count=2):
            ...
            for msg in reversed(messages):
                if isinstance(msg, AIMessage):
                    ...
                    # Stop if we've collected enough AI messages
                    if len(ai_messages) >= count:
                        break
                elif isinstance(msg, HumanMessage):
                    # Break if we encounter a human message while collecting AI messages
                    break
            
            return {
                'last': ai_messages[0] if ai_messages else "",
                'second_last': ai_messages[1] if len(ai_messages) > 1 else "",
            }
        ...
        formatted_prompt = self.reviewer_prompt.format(
            ...
            ai_response_1=recent_ai_messages['second_last'],
            ai_response_2=recent_ai_messages['last']
        )
        try:
            ...
            state['messages'].append(
                AIMessage(
                    content=reviewer_response.final_answer,
                    response_agent="reviewer_agent"
                )
            )
