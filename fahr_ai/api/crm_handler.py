...
class CRMTicketHandler:
    ...
    async def fetch_tickets(self, runtime_config: dict) -> List[Dict]:
        ...
        person_id = userInfo.get("person_id", "")
        full_name = userInfo.get("full_name", "Unknown User")
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                user_email = userInfo.get("email_address", "")
    ...
    async def get_ticket_data(self, runtime_config: dict) -> Dict[str, Dict]:
        ...
        self.logger.info(f"Fetched {len(case_details)} tickets for user {runtime_config['configurable']['userInfo'].get('full_name', 'unknown')}")
