...
from context_aware_splitter import *

os.environ['NO_PROXY'] = '10.254.115.17, 10.254.140.69'
import re

# Step 1: Flip Arabic digit sequences
def fix_arabic_numbers(text):
    def reverse_digits(match):
        return match.group()[::-1]
    return re.sub(r'[\u0660-\u0669]+', reverse_digits, text)

# Step 2: Convert Arabic-Indic digits to English digits
def convert_arabic_digits(text):
    return text.translate(str.maketrans('٠١٢٣٤٥٦٧٨٩', '0123456789'))

# Step 3: Combined function
def normalize_arabic_numbers(text):
    text = fix_arabic_numbers(text)
...
class DocumentIngestionPipeline:
    def __init__(
        ...
        ollama_model=ocr_model,
        use_llm_merging = use_llm_merging
        )
        self.context_splitter = LegalDocumentChunker(config)
        ...
        try:
            if file_extension == '.pdf':
                ...
                try:
                    ...
                    total_text = sum(len(page.page_content) for page in pages)
                    if total_text < 100:  # Threshold for considering as scanned
                        logger.info("Document appears to be scanned, using OCR")
                        pages = self._process_scanned_pdf(str(file_path), document_id, document_name, processed_at)
                    else:
                    ...
                    full_text = "\n".join([normalize_arabic_numbers(page.page_content) for page in pages])
                    # Combine metadata if needed (here we just take the metadata from the first page)
                    metadata = pages[0].metadata if pages else {}
                    # Create a single Document
                    document = Document(page_content=full_text, metadata=metadata)
                    # Final result: list with one document
                    documents = [document]
                    return documents
                except Exception as e:
    ...
    def ingest_documents(self, file_paths: List[str]) -> int:
        ...
        for file_path in file_paths:
            ...
            for doc in documents:
                chunks = self.context_splitter.chunk_document(doc)
