...
class TokenAwareSemanticChunker(SemanticChunker):
    def __init__(self, *args, model_name=settings["tokenizer_model"], max_tokens=450, **kwargs):
        self.max_tokens = max_tokens

        # Set tokenizer BEFORE calling super().__init__()
    ...
    def split_text(self, text: str) -> list[str]:
        ...
        for chunk in initial_chunks:
            ...
            if len(tokens) <= self.max_tokens:
                ...
                logger.warning(f"Chunk over token limit: {len(tokens)} tokens. Splitting it.")
    ...
    def create_documents(self, texts: list[str], metadatas: list[dict]) -> list[dict]:
        docs = []
        for text, metadata in zip(texts, metadatas):
            for chunk in self.split_text(text):
                token_count = len(self.tokenizer.encode(chunk, add_special_tokens=False))
                if token_count > self.max_tokens:
                    logger.error(f"CHUNK STILL TOO BIG after split: {token_count} tokens!!!")
                docs.append({
                    "text": chunk,
...
class IngestionPipeline:
    ...
    def __init__(self):
        ...
        self.semantic_chunker = TokenAwareSemanticChunker(
            embeddings=self.vector_store.embeddings,
            model_name=settings["tokenizer_model"],
            max_tokens=450 # keep lesss than num ctz with some margin always
         )
    ...
    def process_document(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for page_number in extracted_text.keys():
                ...
                page_chunks = self.semantic_chunker.create_documents([combined_text], [base_metadata])
            ...
            if chunks:
                try:
                    for c in chunks:
                        token_count = len(self.semantic_chunker.tokenizer.encode(c['text'], add_special_tokens=False))
                        assert token_count <= settings["num_ctx"], f"Oversized chunk detected: {token_count} tokens"
